...
async function saveKeyWeight() {
  ...
  try {
    await keysApi.updateKeyWeight(editingKey.value.id, editingWeight.value);
    editingKey.value.weight = editingWeight.value;
    window.$message.success(t("keys.weightUpdated"));
...
<template>
  ...
  <!-- 权重编辑对话框 -->
  <n-modal v-model:show="weightDialogShow" preset="dialog" :title="t('keys.editKeyWeight')">
    <div style="margin-bottom: 8px; color: var(--text-secondary); font-size: 13px">
      {{ t("keys.keyWeightDescription") }}
    </div>
    <n-input-number
